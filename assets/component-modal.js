...
/* harmony import */ var feather_icons__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(508);
...
class EnvyModal extends HTMLElement {
  constructor() {
    super();
    this.$element = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this);
    this.modalId = this.dataset.modalId;
    this.blockId = this.dataset.blockId;
    this.$modalCloseButton = this.$element.find('.modal__close-btn');
    this.$modalPrompt = this.$element.find('#modal__prompt');
    this.$modalWrapper = this.$element.find('.modal__background');
    this.$modalContent = this.$element.find('#modal__content');
    this.$modalFocusable = null;
  }
  ...
  initializeModal() {
    // Make all modal content un-tabbable until it is open.
    this.$modalFocusable = this.$modalWrapper.find('button, a, input, select, textarea, iframe');

    this.$modalFocusable.each(function () {
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).attr('tabindex', '-1');
    });

    document.querySelector(`[data-modal-open-btn="${this.modalId}"]`).addEventListener('click', this.modalOpen.bind(this));

    document.querySelector(`[data-modal-open-btn="${this.modalId}"]`).addEventListener('keydown', this.focusInModal.bind(this));

    this.$modalCloseButton.on('click', this.modalClose.bind(this));

    this.$modalCloseButton.on('keydown', this.keyModalClose.bind(this));

    this.$modalWrapper.on('click', this.backgroundModalClose.bind(this));

    if (this.blockId && window.Shopify.designMode) {
  ...
  modalOpen(e) {
    e.preventDefault();
    this.$modalPrompt.addClass('modal--visible');
    this.$modalWrapper.addClass('modal--visible');

    // Lock scrolling
    const scrollY = window.scrollY;
    document.body.style.position = 'fixed';
    document.body.style.top = `-${scrollY}px`;
    document.body.style.overflowY = 'scroll'; // Prevent horizontal page jump

    // Enable focus and trap focus in modal
    this.$modalFocusable.each(function () {
  ...
  focusInModal() {
    // When using keyboard, focus on first focusable element in modal
    setTimeout(() => this.$modalFocusable[0].focus(), 200);
  }

  trapFocus() {
    var inputs = jquery__WEBPACK_IMPORTED_MODULE_0___default().makeArray(this.$modalFocusable);
  ...
  modalClose() {
    this.$modalPrompt.removeClass('modal--visible');
    this.$modalWrapper.removeClass('modal--visible');
    ...
    this.$modalCloseButton.attr('tabindex', '-1');
  }

  // Close modal using enter key
  keyModalClose(e) {
    if (e.which === 13) {
      this.modalClose();
      setTimeout(() => document.querySelector(`[data-modal-open-btn="${this.modalId}"]`).focus(), 200) // Return focus to modal open button
    }
  }

  // Close modal when background is clicked
  backgroundModalClose(e) {
...
customElements.define('envy-modal', EnvyModal);
